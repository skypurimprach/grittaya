<template>
    <div class="h-32">
        <h1 class="text-center text-xl mt-5">
            สนใจสั่งซื้อสินค้ากฤตธยา และปรึกษาเรื่องงานบุญ
            หรือจัดพิธีบวงสรวงต่างๆ
        </h1>
        <br />
        <div class="nav flex justify-center items-center gap-4">
            <img class="img w-12 h-12" src="/images/icon facebook.webp" />
            <h1 class="h1 text-base">
                กฤตธยา รับจัดงานพิธี บายศรีโบราณ<br />เครื่องบวงสรวงและงานฝีกมือเอกลักษณ์ไทย
            </h1>
            <img class="img w-8 h-8" src="/images/icon line.png" />
            <h1 class="h1 text-base">Grittaya</h1>
            <img class="img w-8 h-8" src="/images/icon tel.png" />
            <h1 class="h1 text-base">097-9905388, 081-9958788</h1>
        </div>
    </div>
</template>

<script setup lang="ts"></script>

<style scoped></style>
